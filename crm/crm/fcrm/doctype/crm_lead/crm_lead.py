...
class CRMLead(Document):
	...
	def always_create_organization(self):
		...
		# (autoname: field:organization_name) - so the name IS the primary
		# key, and two orgs literally cannot share one. Since we're
		# deliberately always creating fresh records (no dedup), give the
		# FIELD VALUE itself a disambiguating suffix when a same-named
		# Organization already exists - e.g. "Accenture (2)" - so Frappe's
		# own autoname derives a unique name naturally.
		# (Note: setting organization.name directly here does NOT work -
		# Frappe re-derives name from organization_name on insert regardless
		# of any name already set, since the doc is __islocal.)
		unique_name = self.organization
		suffix = 2
		while frappe.db.exists("CRM Organization", unique_name):
			unique_name = f"{self.organization} ({suffix})"
			suffix += 1
		organization.organization_name = unique_name
