  ...
  <div class="flex-1 overflow-y-auto p-5">
    ...
    <div v-else class="space-y-4 max-w-4xl">
      ...
      >
        <!-- Order Header -->
        <div
          class="flex justify-between items-start p-4 hover:bg-surface-gray-2 cursor-pointer transition-colors"
        ...
        >
          <div class="flex flex-col gap-1">
            <div class="flex items-center gap-2">
              ...
              <span
                v-if="order.lab_required"
                class="text-xs px-2 py-0.5 rounded bg-surface-orange-1 text-ink-orange-3"
              >{{ __('Lab') }}</span>
              <span
                v-if="order.training_required"
                class="text-xs px-2 py-0.5 rounded bg-surface-blue-1 text-ink-blue-3"
              >{{ __('Training') }}</span>
            </div>
        ...
        <div v-if="expandedOrders.has(order.name)" class="border-t border-outline-gray-2">
          <div class="px-4 py-3 bg-surface-gray-1">
            ...
            <div class="overflow-x-auto">
              <table class="w-full text-sm">
                <thead class="bg-surface-gray-2">
                  <tr>
                    <th class="text-left px-4 py-2 text-xs text-ink-gray-5 font-medium">{{ __('Item') }}</th>
                    <th class="text-left px-4 py-2 text-xs text-ink-gray-5 font-medium">{{ __('Description') }}</th>
                    <th class="text-right px-4 py-2 text-xs text-ink-gray-5 font-medium">{{ __('Qty') }}</th>
                    <th class="text-right px-4 py-2 text-xs text-ink-gray-5 font-medium">{{ __('Rate') }}</th>
                    <th class="text-right px-4 py-2 text-xs text-ink-gray-5 font-medium">{{ __('Amount') }}</th>
                    <th class="text-left px-4 py-2 text-xs text-ink-gray-5 font-medium">{{ __('Delivery Date') }}</th>
                    <th class="text-left px-4 py-2 text-xs text-ink-gray-5 font-medium">{{ __('Status') }}</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-outline-gray-1">
                  <tr
                    v-for="(item, idx) in order.delivery_orders"
                    :key="idx"
                    class="hover:bg-surface-gray-1"
                  >
                    <td class="px-4 py-2.5 font-medium text-ink-gray-9">{{ item.item }}</td>
                    <td class="px-4 py-2.5 text-ink-gray-6 max-w-xs truncate">{{ item.description || '—' }}</td>
                    <td class="px-4 py-2.5 text-right text-ink-gray-9">{{ item.qty }}</td>
                    <td class="px-4 py-2.5 text-right text-ink-gray-9">{{ formatCurrency(item.rate) }}</td>
                    <td class="px-4 py-2.5 text-right font-medium text-ink-gray-9">{{ formatCurrency(item.amount) }}</td>
                    <td class="px-4 py-2.5 text-ink-gray-6">{{ item.delivery_date ? formatDate(item.delivery_date) : '—' }}</td>
                    <td class="px-4 py-2.5">
                      <span
                        class="px-2 py-0.5 text-xs rounded-full"
                        :class="deliveryStatusClass(item.status)"
