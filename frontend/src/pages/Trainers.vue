<template>
  ...
  <!-- Filters & Search Bar -->
  <div class="flex items-center gap-3 px-5 py-3 border-b border-outline-gray-1">
    <div class="relative flex-1 max-w-sm">
      <input
        v-model="searchQuery"
        type="text"
        :placeholder="__('Search trainers...')"
        class="w-full rounded-md border border-outline-gray-2 bg-surface-gray-1 py-1.5 pl-8 pr-3 text-sm text-ink-gray-8 placeholder-ink-gray-4 focus:border-outline-gray-4 focus:bg-surface-white focus:outline-none"
        @input="onSearch"
      />
    ...
    >
      ...
      <option value="Partially Available">{{ __('Partially Available') }}</option>
  ...
  <div class="flex-1 overflow-auto">
    ...
    <table v-else class="w-full text-sm">
      <thead class="sticky top-0 z-10 bg-surface-gray-2 border-b border-outline-gray-2">
        <tr>
          ...
          <th
            ...
            :key="col.key"
            class="text-left px-3 py-2.5 text-xs font-medium text-ink-gray-5 whitespace-nowrap cursor-pointer hover:text-ink-gray-8 select-none"
          ...
          <th class="px-3 py-2.5 text-xs font-medium text-ink-gray-5">
            <button class="flex items-center gap-1 text-ink-blue-3 hover:text-ink-blue-4" @click="showColumnManager = true">
      ...
      <tbody class="divide-y divide-outline-gray-1">
        <tr
          ...
          :key="trainer.name"
          class="hover:bg-surface-gray-1 group cursor-pointer"
        ...
        >
          ...
          <!-- Trainer Name -->
          <td class="px-3 py-2.5 font-medium text-ink-gray-9 whitespace-nowrap">
            <div class="flex items-center gap-2">
              <div class="h-7 w-7 flex-shrink-0 rounded-full bg-surface-blue-1 flex items-center justify-center text-xs font-semibold text-ink-blue-3">
          ...
          <!-- Dynamic columns -->
          <td v-for="col in visibleColumns.slice(1)" :key="col.key" class="px-3 py-2.5 text-ink-gray-7 whitespace-nowrap max-w-xs">
            <!-- LinkedIn -->
            <template v-if="col.key === 'linkedin_profile'">
              <a
                v-if="trainer.linkedin_profile"
                :href="trainer.linkedin_profile.startsWith('http') ? trainer.linkedin_profile : 'https://' + trainer.linkedin_profile"
                target="_blank"
            ...
            <template v-else-if="col.key === 'status'">
              <span class="px-2 py-0.5 text-xs rounded-full" :class="statusClass(trainer.status)">
                {{ trainer.status || '—' }}
              </span>
            </template>
            <!-- Availability badge -->
            <template v-else-if="col.key === 'availability'">
              <span class="px-2 py-0.5 text-xs rounded-full" :class="availabilityClass(trainer.availability)">
            ...
            <template v-else-if="col.key === 'remarks'">
              <span class="truncate block max-w-xs" v-html="stripHtml(trainer.remarks)" />
            </template>
            <!-- Default -->
            <template v-else>
              {{ trainer[col.key] || '—' }}
          ...
          <td class="px-3 py-2.5" @click.stop>
            <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button class="p-1 rounded hover:bg-surface-gray-3 text-ink-gray-5 hover:text-ink-gray-8" @click.stop="openEditModal(trainer)">
                <svg class="h-3.5 w-3.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div v-if="trainers.length > 0" class="flex items-center justify-between px-5 py-3 border-t border-outline-gray-1">
      <div class="text-sm text-ink-gray-5">
        {{ __('Showing {0}–{1} of {2}', [((page - 1) * pageLength) + 1, Math.min(page * pageLength, total), total]) }}
      </div>
      <div class="flex items-center gap-2">
  ...
  <div v-if="selected.length > 0" class="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 rounded-lg bg-surface-gray-8 px-4 py-2.5 shadow-xl">
    ...
    <button class="text-ink-gray-4 hover:text-white" @click="selected = []">✕</button>
  </div>

  <!-- Create / Edit Modal -->
  <Dialog v-model="showModal" :options="{ size: '2xl', title: editingTrainer ? __('Edit Trainer') : __('Add Trainer') }">
    <template #body-content>
      <div class="grid grid-cols-2 gap-4 p-1">
        <div>
          ...
          <input v-model="form.trainer_name" type="text" class="form-input w-full rounded-md border border-outline-gray-2 px-3 py-1.5 text-sm focus:outline-none focus:border-outline-gray-4" />
        </div>
        <div>
          <label class="text-xs font-medium text-ink-gray-6 mb-1 block">{{ __('Phone Number') }}</label>
          <input v-model="form.phone" type="text" class="form-input w-full rounded-md border border-outline-gray-2 px-3 py-1.5 text-sm focus:outline-none focus:border-outline-gray-4" />
        </div>
        <div>
          <label class="text-xs font-medium text-ink-gray-6 mb-1 block">{{ __('Email') }}</label>
          <input v-model="form.email" type="email" class="form-input w-full rounded-md border border-outline-gray-2 px-3 py-1.5 text-sm focus:outline-none focus:border-outline-gray-4" />
        </div>
        <div>
          <label class="text-xs font-medium text-ink-gray-6 mb-1 block">{{ __('LinkedIn Profile') }}</label>
          <input v-model="form.linkedin_profile" type="text" placeholder="https://linkedin.com/in/..." class="form-input w-full rounded-md border border-outline-gray-2 px-3 py-1.5 text-sm focus:outline-none focus:border-outline-gray-4" />
        </div>
        <div>
          <label class="text-xs font-medium text-ink-gray-6 mb-1 block">{{ __('Location') }}</label>
          <input v-model="form.location" type="text" class="form-input w-full rounded-md border border-outline-gray-2 px-3 py-1.5 text-sm focus:outline-none focus:border-outline-gray-4" />
        </div>
        <div>
          <label class="text-xs font-medium text-ink-gray-6 mb-1 block">{{ __('Technology Expert In') }}</label>
          <input v-model="form.technology_expert_in" type="text" placeholder="e.g. SAP, AWS, Python..." class="form-input w-full rounded-md border border-outline-gray-2 px-3 py-1.5 text-sm focus:outline-none focus:border-outline-gray-4" />
        </div>
        <div>
          <label class="text-xs font-medium text-ink-gray-6 mb-1 block">{{ __('Skill Level') }}</label>
          <select v-model="form.skill_level" class="form-input w-full rounded-md border border-outline-gray-2 px-3 py-1.5 text-sm focus:outline-none focus:border-outline-gray-4">
        ...
        <div>
          ...
          <input v-model="form.experience" type="text" placeholder="e.g. 5 years" class="form-input w-full rounded-md border border-outline-gray-2 px-3 py-1.5 text-sm focus:outline-none focus:border-outline-gray-4" />
        </div>
        <div>
          <label class="text-xs font-medium text-ink-gray-6 mb-1 block">{{ __('Availability') }}</label>
          <select v-model="form.availability" class="form-input w-full rounded-md border border-outline-gray-2 px-3 py-1.5 text-sm focus:outline-none focus:border-outline-gray-4">
            <option value="">{{ __('Select') }}</option>
        ...
        <div>
          ...
          <select v-model="form.status" class="form-input w-full rounded-md border border-outline-gray-2 px-3 py-1.5 text-sm focus:outline-none focus:border-outline-gray-4">
        ...
        <div>
          ...
          <input v-model="form.commercial" type="number" class="form-input w-full rounded-md border border-outline-gray-2 px-3 py-1.5 text-sm focus:outline-none focus:border-outline-gray-4" />
        </div>
        <div>
          <label class="text-xs font-medium text-ink-gray-6 mb-1 block">{{ __('Company') }}</label>
          <input v-model="form.company" type="text" class="form-input w-full rounded-md border border-outline-gray-2 px-3 py-1.5 text-sm focus:outline-none focus:border-outline-gray-4" />
        </div>
        <div class="col-span-2">
          <label class="text-xs font-medium text-ink-gray-6 mb-1 block">{{ __('Remarks') }}</label>
          <textarea v-model="form.remarks" rows="3" class="form-input w-full rounded-md border border-outline-gray-2 px-3 py-1.5 text-sm focus:outline-none focus:border-outline-gray-4 resize-none" />
        </div>
      </div>
      <ErrorMessage v-if="formError" class="mt-3" :message="formError" />
    </template>
    <template #actions>
  ...
  <Dialog v-model="showColumnManager" :options="{ size: 'sm', title: __('Manage Columns') }">
    <template #body-content>
      <div class="space-y-2 p-1">
        <div v-for="col in allColumns" :key="col.key" class="flex items-center gap-2">
          <input type="checkbox" :id="col.key" v-model="col.visible" class="rounded" />
          <label :for="col.key" class="text-sm text-ink-gray-8 cursor-pointer">{{ col.label }}</label>
        </div>
      </div>
    </template>
    <template #actions>
  ...
  <Dialog v-model="showDeleteDialog" :options="{ size: 'sm', title: __('Delete Trainer') }">
    <template #body-content>
      <p class="text-sm text-ink-gray-7 p-1">
        {{ __('Are you sure you want to delete "{0}"?', [deletingTrainer?.trainer_name]) }}
      </p>
    </template>
    <template #actions>
...
import { Breadcrumbs, Button, Dialog, ErrorMessage, call, toast } from 'frappe-ui'
...
function prevPage() {
  if (page.value > 1) { page.value--; loadTrainers() }
}

function nextPage() {
  if (page.value * pageLength.value < total.value) { page.value++; loadTrainers() }
}

function toggleSelectAll() {
  selected.value = selectAll.value ? trainers.value.map(t => t.name) : []
}
...
function openEditModal(trainer) {
  ...
  Object.assign(form, {
    ...
    technology_expert_in: trainer.technology_expert_in || '',
    skill_level: trainer.skill_level || '',
    experience: trainer.experience || '',
    availability: trainer.availability || '',
    status: trainer.status || 'Active',
    commercial: trainer.commercial || '',
...
async function submitForm() {
  if (!form.trainer_name) {
    formError.value = __('Trainer Name is mandatory')
    return
  }
  saving.value = true
  try {
    if (editingTrainer.value) {
      await call('crm.api.trainers.update_trainer', {
        name: editingTrainer.value.name,
        trainer: JSON.stringify({ ...form }),
      })
      toast.success(__('Trainer updated successfully'))
    } else {
      await call('crm.api.trainers.create_trainer', {
        trainer: JSON.stringify({ ...form }),
      })
...
async function doDelete() {
  ...
  try {
    ...
    toast.error(__('Failed to delete trainer'))
  } finally {
...
function statusClass(status) {
  return {
    'bg-surface-green-1 text-ink-green-3': status === 'Active',
    'bg-surface-gray-2 text-ink-gray-6': status === 'Inactive',
    'bg-surface-red-1 text-ink-red-3': status === 'Blacklisted',
  }
}

function availabilityClass(availability) {
  return {
    'bg-surface-green-1 text-ink-green-3': availability === 'Available',
    'bg-surface-orange-1 text-ink-orange-3': availability === 'Partially Available',
    'bg-surface-red-1 text-ink-red-3': availability === 'Not Available',
  }
}
